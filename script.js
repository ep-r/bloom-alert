// ==============================================================================
// VERSÃO FINAL E CORRIGIDA DO SCRIPT.JS (COM SISTEMA DE TRADUÇÃO COMPLETO)
// ==============================================================================

// 1. INICIALIZAÇÃO E VARIÁVEIS GLOBAIS
let currentLanguage = 'en'; // Começa em Inglês
...
if (geeTileUrl && geeTileUrl.startsWith('https://')) {
    geeLayer = L.tileLayer(geeTileUrl, {
        ...
        // **** OTIMIZAÇÕES DE PERFORMANCE MELHORADAS ****

        // Não tenta carregar tiles enquanto o utilizador está a arrastar o mapa.
        // Espera que o utilizador pare para carregar, o que torna a navegação mais suave.
        updateWhenIdle: true,

        // Não tenta carregar tiles de zoom intermédio durante a animação de zoom.
        updateWhenZooming: false,

        // **AUMENTA O BUFFER:** Pede ao mapa para carregar mais tiles à volta da
        // área visível. Usa um pouco mais de dados, mas torna o ato de mover o mapa (panning)
        // quase instantâneo, pois os tiles vizinhos já estão pré-carregados.
        keepBuffer: 8,

        // **DEFINE UM ZOOM MÁXIMO:** A nossa análise de satélite não tem detalhe
        // infinito. Com esta opção, a camada de calor deixará de ser mostrada
        // a partir do nível de zoom 12, evitando recarregamentos desnecessários
        // quando o utilizador faz muito zoom para ver uma rua, por exemplo.
        maxZoom: 20
...
async function updateSidebar(lat, lon) {
    ...
    try {
        ...
        const [weatherResponse, locationResponse] = await Promise.all([fetch(weatherApiUrl), fetch(locationApiUrl)]);
...
function calculatePollenIndex(temp, humidity, wind) {
    let tempScore = Math.max(0, Math.min(3, (temp - 10) / 5));
    let humidityScore = humidity < 40 ? 3 : (humidity < 70 ? 2 : 1);
    let windScore = wind < 5 ? 1 : (wind < 20 ? 3 : 2);
    let pollenIndex = tempScore + humidityScore + windScore + Math.random();
    return Math.min(pollenIndex, 10);
}

map.on('click', (e) => updateSidebar(e.latlng.lat, e.latlng.lng));
...
function updateDetailsModal(weatherData, locationInfo, lat, lon, pollenIndex) {
    ...
    document.getElementById('recommendations-grid').innerHTML = recommendations.map(rec => `<div class="rec-item"><i class="fas ${rec.icon} rec-icon" style="color: var(--accent-color);"></i><div class="rec-text-content"><p class="rec-title">${rec.title}</p><p class="rec-value">${rec.value}</p></div><div class="status-dot ${rec.color}"></div></div>`).join('');
    ...
    for(let i = 1; i <= 5; i++) {
        ...
        if (hourIndex > -1) {
            ...
            hourlyContainer.innerHTML += `<div class="hourly-item"><strong>${timeStr}</strong><p>${temp_hour}°C</p><i class="fas fa-tint icon-green"></i><p>${rainProb}%</p></div>`;
        }
    }
}
...
function addMessageToChatScreen(message, sender) { const div = document.createElement('div'); div.className = `chat-message ${sender}-message`; div.textContent = message; chatScreen.appendChild(div); chatScreen.scrollTop = chatScreen.scrollHeight; }
// O "cérebro" melhorado do nosso chatbot simulado
function getGeminiChatResponse(userInput) {
    const lowerInput = userInput.toLowerCase();
    
    // Palavras-chave para cada tópico (em inglês e português)
    ...
    // Verifica se a pergunta do utilizador contém alguma das palavras-chave
    ...
    // Respostas que usam os dados do tempo em tempo real
...
// ROUTING
// ==============================================================================
// **** FUNCIONALIDADE DE ROTA "AR LIMPO" (VERSÃO CORRIGIDA) ****
// ==============================================================================
...
// Lógica para mostrar/esconder o painel
openRoutingBtn.addEventListener('click', () => routingPanel.classList.add('open'));
...
// Evento do formulário de rotas ATUALIZADO
routingForm.addEventListener('submit', async (event) => {
    ...
    clearRoutes(); // Limpa as rotas antigas
    ...
    // **** CORREÇÃO PRINCIPAL: Guarda as novas rotas para poderem ser removidas ****
